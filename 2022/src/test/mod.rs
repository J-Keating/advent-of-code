    ...
    for line in lines {
        ...
        if &line[0..2] == "$ " {
            if collecting {
                if !path.is_empty()
                {
                    println!("{}: {}", path.iter().fold(String::new(), |res, dir| res + "/" + &(dir.as_ref().unwrap().name)), path.last().unwrap().as_ref().size);
                }
                collecting = false;
            }
            if line == "$  " {
            ...
            let ref_cell = path.last().unwrap();
            let mut top_dir = ref_cell.borrow_mut();
            let (a, b) = line.split(" ").collect_tuple().unwrap();
            match a {
                "dir" => {
                    top_dir.children.push(Directory { name: b.to_string(), size: 0, children: Vec::new() } );
                }
                _ => {
                    top_dir.size += a.parse::<u32>().unwrap();
                }
            }
        }
    }
